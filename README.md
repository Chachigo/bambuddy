...
- **Chamber light follows the camera** — optional per printer (off by default, videotape button in the Controls row, next to the one above): the chamber light comes on when someone opens this printer's built-in camera and goes off when the last viewer leaves. Only ever undoes its own doing — a light that was already on when you started watching is left alone, and a print that starts mid-view keeps it on after you close the tab
